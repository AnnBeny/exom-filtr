...
- Uživatel nahraje 1 nebo více souborů ve formátu `txt` nebo `tsv` do celkové maximální velikosti 200 MB.
- Aplikace je načítá jako tabulku se separátorem tab (`\t`) pomocí `pandas.read_csv`.

## 2) Výběr genů

- V levém panelu vypíše nebo nakopíruje seznam hledaných genů (pod sebe, bez čárky)
- Klikne na **Použít vybrané geny**.
- Aplikace si seznam uloží do `st.session_state` (aby se neztratil při rerunu).

### 3) Kontrola proti referenčnímu seznamu
...
- Porovná zadané geny s referenčním seznamem a zobrazí:
  ...
  - **success**, pokud v nich jsou všechny
...
- Zobrazí se:
  ...
  - filtrovaná data (spodní tabulka)
- a nabídne se stažení `txt` nebo `tsv`
...
- jsou tři - modrooranžová, růžová a černá. Každou určuje subor `config-*.toml` ve složce `.streamlit` a je potřeba vybrat konkrétní a nastavit v `.bat`/`.desktop`.
